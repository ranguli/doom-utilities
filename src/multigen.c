...
int main(void) {
    ...
    for (i = 0, st = states; i < numstates; i++, st++) {
        fprintf(stenum, "{SPR_%s,%li,%li,%s,%s,%li,%li}", spritename[st->sprite], st->frame, st->tics,
                actionname[st->action], statename[st->nextstate], st->misc1, st->misc2);
        if (i != numstates - 1)
            fprintf(stenum, ",");
        fprintf(stenum, "\t// %s\n", statename[i]);
    }
